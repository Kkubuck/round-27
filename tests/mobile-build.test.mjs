...
test("mobile bundle is complete and fully offline", async () => {
  ...
  assert.match(html, /\.\/story-bg\.png/);
...
test("Capacitor has copied the bundle into the Android project", async () => {
  ...
  await Promise.all([
    ...
    access(new URL("story-bg.png", nativeRoot)),
  ]);
